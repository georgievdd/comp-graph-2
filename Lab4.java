...
public class Lab4 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        log.println("   Пороговый t=50 (≈15 дБ — ХУЖЕ исходного шума!):");
        log.println("     Неожиданно плохой результат. Причина: если центральный пиксель сам является");
        log.println("     импульсным шумом (0 или 255), порог исключает большинство соседей и фильтр");
        log.println("     вырождается. Пороговое усреднение применимо, только если центр не зашумлён.");
        log.println("     Для импульсного шума нужен медианный фильтр (нелинейный, не реализован).");
        log.println();
        ...
        log.println("     Нелинейный ФВЧ. Вычисляет локальный диапазон яркостей.");
        log.println("     Устойчив к мелкому шуму (не применяет операций усреднения).");
        log.println("     Даёт более широкие контуры, чем LoG, но лучше выявляет крупные структуры.");
        log.println("     Размер структурирующего элемента управляет масштабом детектируемых объектов.");
        log.println();
