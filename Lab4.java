...
public class Lab4 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        log.println();
        log.println("=== ВЫВОДЫ: СРАВНЕНИЕ ФИЛЬТРОВ ===");
        log.println();
        log.println("1. ФИЛЬТРАЦИЯ ГАУССОВОГО ШУМА (σ²=400)");
        log.println("   Исходный шум ≈ 18–19 дБ PSNR (зависит от изображения).");
        log.println();
        log.println("   Box 3×3 (≈24–31 дБ):");
        log.println("     Простой усредняющий фильтр. Быстрый. На сложных текстурных изображениях");
        log.println("     (baboon) даёт лучший результат, чем Box 7×7, т.к. мелкие детали не размыты.");
        log.println("     На гладких изображениях (gradient) заметно уступает Box 7×7.");
        log.println();
        log.println("   Box 7×7 (≈23–39 дБ):");
        log.println("     Усиленное размытие. Хорошо работает на плавных изображениях (gradient ≈39 дБ),");
        log.println("     но на богатых деталями изображениях (baboon ≈23 дБ) размывает структуру.");
        log.println();
        log.println("   Гауссов фильтр σ=2 (≈24–35 дБ):");
        log.println("     Взвешенное среднее с гауссовым ядром — лучший баланс: центр весит больше.");
        log.println("     Превосходит Box на большинстве изображений. Рекомендован как стандартный НЧ-фильтр.");
        log.println();
        log.println("   Оптимальный sigma (подбор по PSNR):");
        log.println("     Даёт значительный выигрыш (gradient: σ=5.00 → 45.39 дБ vs 38.78 дБ для σ=2).");
        log.println("     Для текстурных изображений оптимальный sigma мал (baboon: σ=0.75, иначе размытие).");
        log.println("     Вывод: sigma должен соответствовать уровню и типу шума, а не быть фиксированным.");
        log.println();
        log.println("   Билатеральный фильтр (≈25–31 дБ):");
        log.println("     Лучший результат на детальных изображениях (baboon: 25.99 vs 24.76 дБ у Box 3×3).");
        log.println("     На плавных изображениях проигрывает Box 7×7 (gradient: 31 vs 39 дБ), т.к.");
        log.println("     весовая функция диапазона мало улучшает уже однородные области.");
        log.println("     Ключевое преимущество: сглаживает шум ВНУТРИ регионов, не размывая границы.");
        log.println();
        log.println("2. ФИЛЬТРАЦИЯ ИМПУЛЬСНОГО ШУМА (10% соль/перец)");
        log.println("   Box 3×3 (≈22–25 дБ): уменьшает импульсный шум, но не устраняет полностью.");
        log.println("   Box 7×7 (≈22–29 дБ): лучше для импульсного шума на гладких изображениях.");
        log.println("   Гауссов σ=2 (≈23–30 дБ): близок к Box 7×7, хорош для крупных изображений.");
        log.println("   Пороговый фильтр t=50:");
        log.println("     Вычисляет локальное среднее по окну, затем заменяет пиксель средним,");
        log.println("     только если |пиксель − среднее| > T. Незашумлённые пиксели не изменяются.");
        log.println("     При импульсном шуме: соль/перец (0 или 255) резко отличается от локального");
        log.println("     среднего (~128) → заменяется средним. Даёт хороший результат для умеренного");
        log.println("     импульсного шума, не размывая незашумлённые области.");
        log.println();
        log.println("3. ДЕТЕКТИРОВАНИЕ ГРАНИЦ (ФВЧ)");
        log.println("   Лапласиан (4-связный / 8-связный):");
        log.println("     Вторая производная. Быстрый и прост. Реагирует на ВСЕ резкие изменения,");
        log.println("     включая шум → при малейшем зашумлении выход засорён ложными откликами.");
        log.println("     Применяется только на чистых изображениях.");
        log.println();
        log.println("   LoG (Laplacian of Gaussian), σ=2:");
        log.println("     Сначала гауссово размытие подавляет шум, затем лапласиан выделяет края.");
        log.println("     Значительно устойчивее к шуму, чем простой лапласиан.");
        log.println("     Нулевые пересечения (zero-crossing) дают тонкие замкнутые контуры.");
        log.println();
        log.println("   Морфологический градиент (дилатация − эрозия):");
        log.println("     Нелинейный детектор границ: вычисляет локальный диапазон яркостей (max − min).");
        log.println("     Даёт более широкие контуры, чем LoG; размер SE управляет масштабом структур.");
        log.println("     Чувствителен к импульсному шуму: одиночный яркий/тёмный пиксель становится");
        log.println("     значением max или min, завышая градиент в этой точке.");
        log.println("     Рекомендация: применять на изображении, предварительно очищенном медианным фильтром.");
        log.println();
        log.println("4. СВОДНАЯ ТАБЛИЦА РЕКОМЕНДАЦИЙ");
        log.println("   +------------------------------+------------------------------------------+");
        log.println("   | Задача                       | Рекомендуемый фильтр                     |");
        log.println("   +------------------------------+------------------------------------------+");
        log.println("   | Гауссов шум, детальн. изобр. | Билатеральный или Гауссов (подобр. σ)    |");
        log.println("   | Гауссов шум, гладк. изобр.   | Box 7×7 или Гауссов (большой σ)          |");
        log.println("   | Импульсный шум (соль/перец)  | Медианный (нелинейный); Box как паллиатив|");
        log.println("   | Детектирование границ        | LoG + zero-crossing (тонкие замкн. края) |");
        log.println("   | Грубые края / объекты        | Морфологический градиент                 |");
        log.println("   | Повышение резкости           | Unsharp masking (на предв. сглаженном)   |");
        log.println("   | Быстрая обработка            | Box через интегральное изображение O(1)  |");
        log.println("   +------------------------------+------------------------------------------+");
        log.close();
